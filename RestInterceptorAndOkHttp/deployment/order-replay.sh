...
REPLAY_RESPONSE="$(curl -X POST http://demo.dev.cubecorp.io/rs/start/$RECORDING_ID \
  -d 'instanceId=test-order&templateSetVer=DEFAULT&userId=OrderUser&endPoint=http://a8d9f3d8c1a9311ea9b500234fefd3a9-1911613861.us-east-2.elb.amazonaws.com:8080' \
  -H 'Content-Type: application/x-www-form-urlencoded' )"

sleep 20

REPLAY_ID=$(echo $REPLAY_RESPONSE |  sed 's/^.*"id":"\([^"]*\)".*/\1/')
